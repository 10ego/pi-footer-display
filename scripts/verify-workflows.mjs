...
export function verifyWorkflowSources({ pullRequest, release, packageJson, allWorkflows }) {
	...
	invariant(!/permission-issues:/.test(releaseJob), "App token must not request unused Issues permission");
	...
	assertIncludes(validate, "npm test", "validate must run the complete test suite");
	assertIncludes(validate, "npm run verify:release-version -- --expected", "validate must bind metadata to the released version");
	assertIncludes(validate, "npm run verify:package", "validate must inspect the release package");
	assertIncludes(validate, "Check npm package availability", "validate must prove npm publication state");
	...
	invariant(occurrences(packageJob, /^\s+npm pack\b/gm) === 1, "package job must build exactly one lifecycle-script-disabled tarball");
	assertIncludes(packageJob, "persist-credentials: false", "package checkout must not persist credentials");
	assertIncludes(packageJob, 'artifact_name="npm-package-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}-${EXPECTED_VERSION}-${EXPECTED_COMMIT_SHA}"', "artifact identity must bind run, attempt, version, and commit");
	...
	assertIncludes(publish, '[[ "$workflow_run_id" == "$GITHUB_RUN_ID" ]] || exit 1', "publish must bind server artifact metadata to the current run");
	assertIncludes(publish, "$PACKAGE_NAME@$VERSION appeared after preparation; refusing publication.", "publish must recheck exact npm version absence immediately before publication");
	...
	invariant(packageJson.scripts?.["test:tooling"]?.includes("tests/tooling/workflows.test.mjs"), "workflow tests must run in test:tooling");
}
